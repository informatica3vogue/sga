...
$sql ="SELECT us.id_usuario,ar.id_dependencia,DATE_FORMAT(so.fecha, '%d-%m-%Y %H:%i:%s')as fecha,us.nombre,det.cantidad,ar.articulo FROM solicitud_articulo so INNER JOIN usuario us ON so.id_usuario=us.id_usuario INNER JOIN detalle_solicitud det ON det.id_solicitud_articulo=so.id_solicitud_articulo INNER JOIN articulo ar ON det.id_articulo=ar.id_articulo WHERE us.id_usuario=:txtUsuario AND so.fecha BETWEEN :txtDesde AND DATE_ADD(:txtHasta, INTERVAL 1 DAY) and ar.id_dependencia=:id_dependencia";
$param_list=array('txtUsuario', 'txtDesde', 'txtHasta','id_dependencia');
...
<tr><td>Reporte de insumos por solicitud del usuario: '.$result["items"][0]["nombre_usuario"].'.</td></tr>
</table><hr></header>';
