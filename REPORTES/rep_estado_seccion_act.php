...
$sql = "SELECT a.id_actividad, a.referencia, DATE_FORMAT(a.fecha_procesamiento, '%d-%m-%Y %H:%i:%s') AS fecha_procesamiento, a.dependencia_origen, a.solicitante, a.requerimiento, DATE_FORMAT(a.fecha_finalizacion, '%d-%m-%Y %H:%i:%s') AS fecha_finalizacion FROM actividad a INNER JOIN seccion s ON s.id_seccion=a.id_seccion WHERE s.id_seccion = :id_seccion AND a.estado = :txtEstado AND a.fecha_procesamiento BETWEEN :txtDesdeUno AND DATE_ADD(:txtHastaUno, INTERVAL 1 DAY) ORDER BY a.fecha_procesamiento DESC";
$param_list=array('id_seccion', 'txtDesdeUno', 'txtHastaUno', 'txtEstado');
$response = $data->query($sql, $params, $param_list);

$tipo = ($params["txtEstado"] == "Finalizado") ? "Finalizadas":"Pendientes";
...
<tr><td>Reporte de actividades '.$tipo.' Desde: '.date("d-m-Y",strtotime($params["txtDesdeUno"])).' Hasta: '.date("d-m-Y",strtotime($params["txtHastaUno"])).'.</td></tr>
</table><hr></header>';
