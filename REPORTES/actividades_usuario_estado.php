...
function header_page(){
    $params = $_POST;
    $data = new data();
	$sql = "SELECT CONCAT(nombre, ' ', apellido) AS nombre_usuario FROM usuario WHERE id_usuario = :txtUsuario"; 
	$response = $data->query($sql, array("txtUsuario" => $params["txtUsuario"]));
        ...
        <header style='height:25mm;border:0px green solid;'>
            <div style='float:left;width:100%;height:25mm;position:absolute;display:inline;border:0px red solid;text-align:center;font-weight:bold;margin-left:0;'>
                <h3>
                  ...
                  [dependencia]<br>
                  Reporte de actividades ".strtolower($params['txtEstado'])."s, del usuario: ".$response['items'][0]['nombre_usuario']." Desde: ".date('d-m-Y',strtotime($params['desde']))." Hasta: ".date('d-m-Y',strtotime($params['hasta'])).".
                </h3>
            </div>
...
function registros_pendiente(){
    ...
    $i = 0;
    $html = '';
    $sql = "SELECT a.id_actividad, a.referencia, DATE_FORMAT(a.fecha_procesamiento, '%d-%m-%Y %H:%i:%s') AS fecha_procesamiento, a.estado, a.solicitante, a.requerimiento, IF(a.fecha_finalizacion != NULL OR a.fecha_finalizacion!= '', DATE_FORMAT(a.fecha_finalizacion, '%d-%m-%Y %H:%i:%s'), 'No finalizada') AS fecha_finalizacion, IF(d.tipo='Externa', d.dependencia, (SELECT bddep.dependencia FROM bddependencias.dependencia bddep WHERE bddep.id_dependencia=a.id_dependencia_origen)) AS dependencia_origen 
    	FROM actividad a INNER JOIN asignacion asi ON a.id_actividad = asi.id_actividad 
        INNER JOIN seccion s ON a.id_seccion = s.id_seccion 
        INNER JOIN dependencia d ON d.id_dependencia=a.id_dependencia_origen 
        WHERE a.fecha_procesamiento BETWEEN :desde AND DATE_ADD(:hasta, INTERVAL 1 DAY) AND asi.id_usuario = :txtUsuario AND a.estado = :txtEstado ORDER BY a.fecha_procesamiento DESC";
    $param_list=array('desde', 'hasta', 'txtUsuario', 'txtEstado');
