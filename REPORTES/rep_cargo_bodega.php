...
$sql = "SELECT art.id_dependencia,ca.fecha,CONCAT(us.nombre, ' ', us.apellido) as nombre,ca.referencia,art.articulo,art.existencia,ma.marca FROM cargo_bodega ca INNER JOIN articulo art ON ca.id_articulo=art.id_articulo INNER JOIN usuario us ON ca.id_usuario=us.id_usuario INNER JOIN marca ma on ma.id_marca=art.id_marca WHERE us.id_usuario=:txtUsuario AND art.id_dependencia=:id_dependencia AND ca.fecha BETWEEN :txtDesde AND DATE_ADD(:txtHasta, INTERVAL 1 DAY)";
...
<tr><td>Reporte de cargos de bodega del usuario: '.$result["items"][0]["nombre_usuario"].'.</td></tr>
</table><hr></header>';
