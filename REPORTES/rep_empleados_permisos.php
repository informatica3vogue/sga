...
$sql = "SELECT per.id_permiso, per.num_permiso, CONCAT(emp.nombre, ' ',emp.apellido) AS nombre_completo, DATE_FORMAT(per.fecha_dif, '%d-%m-%Y') AS f_dif, DATE_FORMAT(per.fecha_desde, '%d-%m-%Y') AS f_desde, DATE_FORMAT(per.fecha_hasta, '%d-%m-%Y') AS f_hasta, DATEDIFF(per.fecha_hasta,DATE_ADD(per.fecha_desde, INTERVAL -1 DAY)) AS dias, SEC_TO_TIME(TIME_TO_SEC(TIMEDIFF(per.hora_hasta,per.hora_desde))) AS horas,per.fecha_procesamiento FROM permiso per INNER JOIN empleado emp ON per.id_empleado = emp.id_empleado WHERE per.id_empleado =:txtNombreEmpleado AND per.fecha_procesamiento BETWEEN :txtDesdeDD AND DATE_ADD(:txtHastaDD, INTERVAL 1 DAY) ORDER BY f_dif DESC";

$param_list=array('txtNombreEmpleado','txtDesdeDD', 'txtHastaDD');
...
<tr><td>Reporte de permiso del empleado: '.$text.'</td></tr>
</table><hr></header>';
