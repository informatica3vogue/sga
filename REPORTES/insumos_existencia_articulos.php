    ...
    <header style='height:25mm;border:0px green solid;'>
            <div style='float:left;width:100%;height:25mm;position:absolute;display:inline;border:0px red solid;text-align:center;font-weight:bold;margin-left:0;'>
                <h3>
                  ...
                  [dependencia]<br>
                  Reporte de existencia de articulos en bodega
                </h3>
            </div>
...
function registros(){
  ...
  $sql ="SELECT ar.articulo ,ar.descripcion, ar.existencia, ma.marca, ar.id_dependencia FROM articulo ar INNER JOIN marca ma on ar.id_marca=ma.id_marca where ar.id_dependencia=:id_dependencia AND ar.id_categoria = 1 order by ar.articulo";
    ...
    foreach ($response["items"] as $datos) {
      ...
      $html.='
        <tr style="'.$estilos.'">
          ...
          <td><center>'.$datos["existencia"].'</center></td>
        </tr>
      ';
    }
    $html.='
      <tr>
                <th colspan="4" style="color: black; text-align: right;"><center>Total de articulos en bodega: <span style="color: blue;">   <b>'.$response["total"].'</b></span></center></th>
            </tr>
    ';
  }
  return $html;
