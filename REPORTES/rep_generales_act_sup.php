...
$sql = "SELECT id_actividad, referencia, DATE_FORMAT(fecha_procesamiento, '%d-%m-%Y %H:%i:%s') AS fecha_procesamiento, dependencia_origen, solicitante, requerimiento, DATE_FORMAT(fecha_finalizacion, '%d-%m-%Y %H:%i:%s') AS fecha_finalizacion, IF(estado='Finalizado', 'Finalizada','Pendiente') AS estado FROM actividad WHERE fecha_procesamiento BETWEEN :txtDesdeUno AND DATE_ADD(:txtHastaUno, INTERVAL 1 DAY) AND id_seccion=:id_seccion ORDER BY estado, fecha_procesamiento DESC";
$param_list=array('txtDesdeUno', 'txtHastaUno', 'id_seccion');
...
<tr><td>Reporte de actividades Desde: '.date("d-m-Y",strtotime($params["txtDesdeUno"])).' Hasta: '.date("d-m-Y",strtotime($params["txtHastaUno"])).'.</td></tr>
</table><hr></header>';
