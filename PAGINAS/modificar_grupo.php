...
<div class="box">
        <div class="box-header">
            <h2><i class="halflings-icon white user"></i><span class="break"></span>Ingresar nuevos usuarios</h2>
            <div class="box-icon">
                <a href="#" data-rel="tooltip" title="Minimizar" class="btn-minimize"><i class="halflings-icon white chevron-up"></i></a>
            </div>
        </div>
        <div class="box-content">
            <form role="form" method="POST" name="frmGrupoEmp" id="frmGrupoEmp" autocomplete="off" enctype="multipart/form-data">
                <div class="span12">
                    <div class="form-group">
                        <label>Nombre grupo: </label>
                        <input type="hidden" class="form-control" id="id_grupo" name="id_grupo" readonly>
                        <input type="text" class="form-control" id="grupo" name="grupo">           
                        <input type="hidden" class="form-control" id="nombre_grupo" name="nombre_grupo" readonly>              
                    </div>    
                </div>
                <div>
                ...
                <div class="form-actions">
                    <button type="button" id="cancelar" name="cancelar" onClick="location.href='?mod=grupos_creados'" class="btn btn-danger btn-movil pull-left margin-right">Cancelar</button>
                    <button type="button" id="guardar" name="guardar" class="btn btn-movil btn-primary pull-right">Guardar</button>
                </div>
            </form>
        </div>
    </div>
...
function agregar(){
    ...
    if(id_empleado==""){
        bootbox.alert('Seleccione un empleado, por favor', function() {  });
    }else{
        $.ajax({
...
$(document).ready(function () {
    $('#guardar').click(function () {
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: "Json",
            url: 'procesos/memorandum/modificar_grupo.php',
            beforeSend: function () {
...
//funcion que lista en tabla los empleados del grupo
$(document).ready(function () {
    $.ajax({
        url: 'procesos/memorandum/listar_empleados_grupo.php',
        type: 'POST',
        dataType: 'json',
        data: {"id_grupo": <?php echo ($_POST['id']); ?>},
        success: function(response){
            $("#detalle_empleado").html("");
            var i=1;
            ...
            $.each(data, function(index, value){
                if (i == 1) {
                    document.getElementById('id_grupo').value = value.id_grupo;
                    document.getElementById('grupo').value = value.grupo;
                    document.getElementById('nombre_grupo').value = value.grupo;
                };
                $("#detalle_empleado").append("<tr><td width='10%'>"+i+"</td><td width='75%'>"+value.empleado+"</td><td width='14%'><a href='#' class='btn btn-danger' onsubmit='return false' onclick='quitar("+value.id_empleado+")'><i class='halflings-icon white trash'></i></a></td></tr>");
                i++;
            });
        },
        error: function(){
            alert('Hubo un problema al cargar la informacion');
        }
    });  
});
</script>